...
load_avonet <- function() {
  ...
  for (i in 1:11) {
  x <- setNames(list(suppressWarnings(read_xlsx("raw_data/AVONET Supplementary dataset 1.xlsx", sheet = i, ))), names[i])
...
This function is in development. Currently, the function is able to retrieve the set taxon system through the 'taxon_system' argument and add this data to the raw dataset of AvoNet. It can also dynamically group observations by species, genus, family, or order.
...
```{r}
summary_avonet <- extract_avonet(taxon_system = "birdlife", taxon_level = "family")
summary_avonet2 <- extract_avonet(taxon_system = "ebird", taxon_level = "genus")
summary_avonet3 <- extract_avonet(taxon_system = "birdtree", taxon_level = "order")
summary_avonet4 <- extract_avonet(taxon_system = "birdlife", taxon_level = "species")
