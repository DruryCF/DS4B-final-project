...
Function loads, tidies, and applies optional cleaning to AvoNet dataset
...
```{r}
avonet <- avonet_load(filename = "raw_data/avonet_dataset.xlsx",
                      clean = "all")
avonet
```

# extract set functional traits by set taxon system from $raw_data

Function is able to retrieve the set taxon system through the 'taxon_system' argument and add this data to the raw dataset of AvoNet. Through the 'traits' argument, users can specify which traits they would like to extract, if unspecified all variables will remain available. Extract calls the avonet_load() function enabling users to bypass loading the entire database. The clean argument also passes to avonet_load() enabling dynamic cleaning.
...
```{r}
extract <- avonet_extract_intraspecies(filename = "raw_data/avonet_dataset.xlsx", taxon_system = "birdlife", traits = c("sex", ".length"))
extract_clean <- avonet_extract_intraspecies(filename = "raw_data/avonet_dataset.xlsx", taxon_system = "birdlife", traits = c("sex", ".length"), clean = "all")

extract
extract_clean
```

# extract set functional trait averages by from $birdlife, $ebird, $birdtree.

This function selects defined variables from the species average tibbles. These contain geographic data.
...
```{r}
avg <- avonet_extract_interspecies(filename = "raw_data/avonet_dataset.xlsx", traits = c("tail.length", "wing.length"))
avg
```

# summarise function

Function returns summary statistics defined by 'stats' argument for each 'group' value and each numeric variable passed to the function through the 'data' argument. Designed to be used in conjunction with avonet_extract_intraspecies() and avonet_extract_interspecies(). By setting tidy = TRUE, the function will return summary stats in tidy format. Function will always return count data for each group. Note that count data returns the number of observations for that species, not the number of species with values for that trait.
...
```{r}
summary <- avonet_summary(data = extract, group = c("species"), stats = c("mean", "median", "max", "min", "sd"), tidy = FALSE)
summary_tidy <- avonet_summary(data = extract, group = c("species"), stats = c("mean", "median", "max", "min", "sd"), tidy = TRUE)
...
```{r}
summary2 <- avonet_summary(data = avg, group = c("family"), stats = c("mean", "median", "max", "min", "sd"), tidy = FALSE)
summary2
```

Function can group by multiple levels enabling partitioning of species averages by sex.

```{r}
summary3 <- avonet_summary(data = extract, group = c("species", "sex"), stats = c("mean", "sd"), tidy = FALSE)
summary4 <- avonet_summary(data = extract, group = c("species", "sex"), stats = c("mean", "sd"), tidy = TRUE)
...
```{r}
summary_plot <- avonet_summary(data = filter(extract, sex == "M"), group = c("family", "genus", "species", "sex"), stats = c("mean"), tidy = TRUE)
summary_plot
```

# Plot example; designed for poster

```{r}
summary_plot_family <- summary_plot %>% 
  filter(family == "Acanthizidae", genus %in% c("Acanthiza", "Aphelocephala", "Calamanthus", "Crateroscelis", "Gerygone", "Sericornis"))

p <- ggplot(summary_plot_family, mapping = aes(genus, tail.length)) + 
  geom_boxplot(fill = "#90BF96") +
  xlab("genus") +
  ylab("tail length (mm)") +
  labs(title = "Tail Length variation of males in genera of the Acanthizidae family") +
  theme(panel.background = element_rect(fill = "#D5E7D7"),
        plot.background = element_rect(fill = "#D5E7D7", colour = "#09806F", linewidth = 3),
        plot.title = element_text(colour = "#094595"),
        axis.title = element_text(colour = "#094595"),
